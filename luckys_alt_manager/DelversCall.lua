...
local function ProbeQuestXP()
    ...
    DevLog("--- XP Probe start for questID " .. probeID .. " ---")

    if C_QuestLog.GetQuestRewardXP then
        local ok, val = pcall(C_QuestLog.GetQuestRewardXP, probeID)
        DevLog(string.format("  C_QuestLog.GetQuestRewardXP(%d) => ok=%s val=%s",
            probeID, tostring(ok), tostring(val)))
        if ok and type(val) == "number" and val > 0 then
            db.xpPerQuest = val
            DevLog("  ** ACCEPTED xpPerQuest = " .. val)
            return
        end
    else
        DevLog("  C_QuestLog.GetQuestRewardXP does not exist")
    end

    if C_QuestLog.SetSelectedQuest then
        C_QuestLog.SetSelectedQuest(probeID)
        local val = GetQuestLogRewardXP and GetQuestLogRewardXP() or nil
        DevLog(string.format("  SetSelectedQuest(%d) + GetQuestLogRewardXP() => %s",
            probeID, tostring(val)))
        ...
        DevLog("  C_QuestLog.SetSelectedQuest does not exist")
    end

    local numEntries = C_QuestLog.GetNumQuestLogEntries()
    for i = 1, numEntries do
        local info = C_QuestLog.GetInfo(i)
        if info and info.questID == probeID then
            local val = GetQuestLogRewardXP and GetQuestLogRewardXP(i) or nil
            DevLog(string.format("  GetQuestLogRewardXP(logIndex=%d) => %s", i, tostring(val)))
            if val and val > 0 then
                db.xpPerQuest = val
                DevLog("  ** ACCEPTED xpPerQuest = " .. val)
                return
            ...
            if SelectQuestLogEntry then
                ...
                local val2 = GetQuestLogRewardXP and GetQuestLogRewardXP() or nil
                DevLog(string.format("  SelectQuestLogEntry(%d) + GetQuestLogRewardXP() => %s",
                    i, tostring(val2)))
                if val2 and val2 > 0 then
                    db.xpPerQuest = val2
                    DevLog("  ** ACCEPTED xpPerQuest = " .. val2)
                    return
                end
            end
            break
    ...
    for _, qid in ipairs(KNOWN_QUEST_IDS) do
        if inLog[qid] and C_QuestLog.SetSelectedQuest then
            C_QuestLog.SetSelectedQuest(qid)
            local selected = C_QuestLog.GetSelectedQuest and C_QuestLog.GetSelectedQuest() or nil
            local val = GetQuestLogRewardXP and GetQuestLogRewardXP() or nil
            DevLog(string.format("  [iter] SetSelected(%d) selected=%s GetQuestLogRewardXP()=%s",
                qid, tostring(selected), tostring(val)))
            if val and val > 0 then
                db.xpPerQuest = val
                DevLog("  ** ACCEPTED xpPerQuest = " .. val)
                return
    ...
    if C_QuestInfoHelper then
        DevLog("  C_QuestInfoHelper exists — dumping methods:")
        for k, v in pairs(C_QuestInfoHelper) do
            DevLog(string.format("    .%s (%s)", tostring(k), type(v)))
        end
    else
        DevLog("  C_QuestInfoHelper does not exist")
    end

    DevLog("  C_QuestLog methods containing 'XP' or 'Reward':")
    for k, v in pairs(C_QuestLog) do
        local lk = k:lower()
        if lk:find("xp") or lk:find("reward") or lk:find("experience") then
            DevLog(string.format("    .%s (%s)", k, type(v)))
        end
    end

    if GetQuestRewardXP then
        local ok, val = pcall(GetQuestRewardXP, probeID)
        DevLog(string.format("  global GetQuestRewardXP(%d) => ok=%s val=%s",
            probeID, tostring(ok), tostring(val)))
        if ok and type(val) == "number" and val > 0 then
            db.xpPerQuest = val
            DevLog("  ** ACCEPTED xpPerQuest = " .. val)
        ...
        DevLog("  global GetQuestRewardXP does not exist")
    end

    if QuestInfo_GetRewardXP then
        local ok, val = pcall(QuestInfo_GetRewardXP)
        DevLog(string.format("  QuestInfo_GetRewardXP() => ok=%s val=%s",
        ...
        DevLog("  QuestInfo_GetRewardXP does not exist")
    end

    if GetRewardXP then
        local ok, val = pcall(GetRewardXP)
        DevLog(string.format("  GetRewardXP() => ok=%s val=%s",
            tostring(ok), tostring(val)))
        if ok and type(val) == "number" and val > 0 then
            db.xpPerQuest = val
            DevLog("  ** ACCEPTED xpPerQuest = " .. val)
        ...
        DevLog("  GetRewardXP does not exist")
    end

    DevLog("--- XP Probe end — no method returned XP > 0 ---")
end
